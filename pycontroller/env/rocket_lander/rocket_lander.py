...
DEGTORAD = np.pi / 180
XX = RocketLanderState.x.value
YY = RocketLanderState.y.value
X_DOT = RocketLanderState.x_dot.value
Y_DOT = RocketLanderState.y_dot.value
THETA = RocketLanderState.theta.value
THETA_DOT = RocketLanderState.theta_dot.value
LEFT_GROUND_CONTACT = RocketLanderState.left_ground_contact.value
RIGHT_GROUND_CONTACT = RocketLanderState.right_ground_contact.value
...
class RocketLander(gym.Env):
    ...
    def __init__(self, args: Dict, render_mode=None):
        ...
        self.cfg = RocketLanderEnvConfig()
        self.args = RocketLanderUserArgs(**args)
    ...
    def _create_rocket(self, pos_x: float, pos_y: float):
        ...
        for i in [-1, +1]:
            ...
            try:
                sensor_radius = self.cfg.leg_contact_sensor_radius
                leg.CreateFixture(
                    fixtureDef(
                        shape=circleShape(
                ...
                # Fallback: if sensor creation fails, ignore silently
                pass
